...
/// This is cacnonicalized path due to macos behaviour - it creates
/// a dir with path '/var/...' but actually it is '/private/var/...'
///
/// FsEventWatcher uses canonicalized paths
/// and send events with canonicalized paths, tho we need it converted to compare
pub struct TestDir {
...
mod expect {
    ...
    pub trait ExpectedEvents: Debug + FromIterator<ExpectedEvent> {
        ...
        /// Returns
        /// * Ok(None) if the event is unexpected
        /// * Ok(Some(expected)) if the event was matched with the expected one
